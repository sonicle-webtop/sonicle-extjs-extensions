 ...
 */
Ext.define('Sonicle.form.field.tinymce.tool.FormatUnderline', {
	extend: 'Sonicle.form.field.tinymce.tool.base.StateButton',
	alias: ['widget.so-tmcetoolunderline'],
	mixins: {
	...
	initComponent: function() {
		...
		Ext.apply(me, {
			...
			handler: function() {
				me.getHtmlEditor().editorExecuteCommand('Underline');
			}
		});
	...
	bindEditor: function(editor, htmlEditor) {
		...
		if (htmlEditor) {
			...
			editor.formatter.formatChanged('underline', monCbks['onFormatChanged']);
		} else if (editor) {
			// Do nothing...
		}
	},
	...
	privates: {
		onEditorFormatChanged: function(active, data) {
			if (data.format === 'underline') this.toggle(active, false);
		}
	}
});
