...
Ext.define('Sonicle.form.field.TinyMCETextArea', {
    ...
    initEditor: function (height) {
        ...
        me.tinyMCEConfig.setup = function (ed) {
			...
			// This tries to work on blockquote splitting on enter
			ed.on('keydown', function(e) {
				if ((e.charCode == 13 || e.keyCode == 13)) { 
					me.checkBlockQuote(ed,e);
				}
			});
	...
	checkBlockQuote: function(ed,e) {
		...
		if (sel.rangeCount>0) {
            ...
            while(anc.parentElement) {
                if (anc.tagName==="BLOCKQUOTE") bq=anc;
                anc=anc.parentElement;
            }
			if (bq) {
				e.preventDefault();
				this.splitBlockQuote(ed,bq);
				var newNode = ed.selection.getNode();
				ed.selection.setCursorLocation(newNode);				
			}
		}
	},
	...
	splitBlockQuote : function(editor, blockquote) {
		...
		editor.selection.setContent('<span id="sonicle-splitter"></span>');
		var splitElement = editor.dom.get('sonicle-splitter');
    ...
    setSelectionContent: function(html) {
        return this.getEditor().selection.setContent(html);
		//var newNode = ed.selection.getNode();
		//ed.selection.setCursorLocation(newNode);				
    },
	
    setSelectionNode: function(el) {
        return this.getEditor().selection.setNode(el);
		//var newNode = ed.selection.getNode();
		//ed.selection.setCursorLocation(newNode);				
    },
    ...
    setSelectionStyle: function(style) {
        var me=this,
            tsel=me.getEditor().selection;

//        if (!tsel.isCollapsed()) {
            var sel=tsel.getSel();
            if (sel.rangeCount>0) {
                var xspan=me.createNode("span",{ style: style}),
...
//        } else {
//            var xspan=me.createNode("span",{ style: style});
//            xspan.innerHTML='ABCDE';
//            me.setSelectionNode(xspan);
//        }
    },
    
    createNode: function(tag,config) {
        return this.getEditor().dom.create(tag,config);
    },
    
    createRange: function(tag,config) {
        return this.getEditor().dom.createRng();
    },
	...
	//-----------------------------------------------------------------
	openNotification: function(config) {
		return this.getEditor().notificationManager.open(config);
	},
